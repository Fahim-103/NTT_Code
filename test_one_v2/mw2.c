...
/*
 *    uint64_t w_powers_na[256];
    uint64_t w_powers_con_na[256];

    for (int i = 0; i < 256; i++){

        w_powers_na[i] = t->w_powers[4*i];
        w_powers_con_na[i] =  t->w_powers_con[4*i];
    }


    fwd_ntt_ref_harvey(temp2a, 256, t->q, w_powers_na, w_powers_con_na, 8);
    ...
    for (int i = 0; i < 256; i++){
        ...
        printf("%llu %llu %llu %llu ", temp2a[i], temp2b[i], temp2c[i], temp2d[i]);
    }
